...
All plots were created in [R](https://www.r-project.org) with support of different packages. The source of the respective chunks is provided in collapsible sections. The entire source can be copied from the raw version of this chapter through the code tools next to the chapter heading.  

```{r}
#| label: load-pkgs
#| code-summary: "Load R packages and datasets"
#| message: false
#| warning: false

library(tidyr)       # general data transformation
library(dplyr)       # general data transformation
library(purrr)       # data transformation using functions
...
library(misc3d)      # tools for 3D plots
library(rgl)         # creating interactive 3D plots
library(viridisLite) # colour blind-friendly colour schemes

```

The same subset of a real archaeological dataset will be used as example to demonstrate how this data can be treated in different ways. The dataset is available in [the GitHub repository of this book](https://github.com/archmetalDBM/GlobaLID-Edu/tree/main/example_dataset) and was originally published by [@Bode.2008]. To avoid overloading of the plots, the subset is restricted to the archaeological objects and reference data from the region "Northern Eifel", "Sauerland", and "Bergisches Land", all located in the West of Germany.   

```{r}
#| label: load-data
#| code-summary: "Import and filter data"
#| message: false
#| warning: false
...
Roman_case$Region <- factor(Roman_case$Region, levels = c('Northern Eifel','Sauerland','Bergisches Land','Roman object')) # defining display order of regions in plots
...
```{r}
#| label: set-styling
#| code-summary: "Define common style elements"
#| message: false
#| warning: false
...
plt <- plot_ly(data = data, x = ~`206Pb/204Pb`, y = ~`207Pb/204Pb`, 
        ...
        hovertemplate = paste('<i>Region</i>: %{text}<br>',
                              '<b>X</b>: %{x}<br>',
                              '<b>Y</b>: %{y}')
        )
...
#| fig-subcap:
#|   - "Ellipses based on ^206^Pb/^204^Pb vs ^207^Pb/^204^Pb."
#|   - "Ellipses based on ^206^Pb/^204^Pb vs ^208^Pb/^204^Pb." 
#| fig-height: 8
#| fig-width: 10
#| layout-ncol: 2
#| message: false
...
p_207 <- ggplotly(p_207, tooltip = c("x", "y", "fill")) %>% 
  layout(xaxis = list(title = "<sup>206</sup>Pb/<sup>204</sup>Pb"),
         ...
         legend = list(title = "", font = list(size = 10), orientation = "h", xanchor = "center", x = 0.5, y = 1.15)
         )
...
plt <- ggtern(data = data, aes(x = Pb207, y = Pb206, z = Pb208, color = Region, fill = Region))+
  geom_density_tern(data = data[data$Region != "Roman object", ], n = 100, bins=10, alpha = 1) +
